...
asset_manifest=${FAMPNN_ASSET_MANIFEST:-/app/revocompute/model-assets.sha256}
echo "REVODESIGN_STAGE:${TASK_TYPE}"
case "${TASK_TYPE}" in
  fampnn_design)
    checkpoint="${weights}/fampnn_0_3.pt"; verify_model_asset "$asset_manifest" "$weights" "fampnn_0_3.pt" FAMPNN
    ...
    ;;
  fampnn_pack)
    checkpoint="${weights}/fampnn_0_0.pt"; verify_model_asset "$asset_manifest" "$weights" "fampnn_0_0.pt" FAMPNN
    ...
    ;;
  fampnn_score)
    checkpoint="${weights}/fampnn_0_3_cath.pt"; verify_model_asset "$asset_manifest" "$weights" "fampnn_0_3_cath.pt" FAMPNN
