...
import json,sys
for item in json.load(open(sys.argv[1]))['inputs']['ligands']: print(item['path'])
PY
)
receptor=$(task_input receptor)
(( ${#ligands[@]} >= 1 )) || { echo 'AutoDock Vina requires ligand files' >&2; exit 1; }
mkdir -p "$out/prepared"; prepared_receptor="$out/prepared/receptor.pdbqt"
...
import json,sys
m=json.load(open(sys.argv[1])); json.dump({'task_type':'autodock_vina','runtime_network':False,'inputs':m['inputs'],'parameters':m.get('params',{})},open(sys.argv[2],'w'),indent=2); open(sys.argv[2],'a').write('\n')
PY
