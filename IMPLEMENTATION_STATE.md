...
| EvoSplit | Dedicated CUDA 12.6/Torch 2.8 family implemented for commit `6eedfcd5a0551bd1fecf9c51818be19f9e730e19` (Apache-2.0). The offline clustering contract reuses exact ESM-MSA-1b assets, preserves clustered alignments/contact evidence, and has real attention-shape regression coverage. | Build the final post-review SIF, run Task -> SLURM -> Apptainer acceptance, then enable. |
| RFdiffusion2 | Dedicated CUDA 12.1/Torch 2.4 family implemented for canonical commit `d365cbf4db3958814a9f8e4f6f94fa309dfebc2b` (BSD-3-Clause). The official `RFD_173.pt` is provisioned read-only with exact size and SHA-256; motif-scaffold and ligand-binder contracts are separate TaskTypes. | Build/test the SIF, but do not run or enable checkpoint-backed inference until upstream confirms checkpoint-use terms. |
| foundry | Dedicated multi-capability family implemented for commit `b02eed6a6bdf8f44d14a80cc36e3da13c9f2291c`, with separate RFD3, RFD3NA, and RF3 TaskTypes and actual RF3 early-stop artifact handling. | Keep disabled until UW/IPD publishes checkpoint terms covering the intended hosted-use model; then provision, build, and live-test. |
| PPIformer | Dedicated CUDA 11.8/Torch 2.1 family implemented from canonical commit `e324f5f30dd0dae55d194ac6b4d18c772219c3ee`. Zenodo CC BY 4.0 weights are provisioned read-only with exact archive/extracted hashes; ddG and embedding workflows are separate TaskTypes. | Build the final SIF, run both smoke cases through SLURM, then enable. |
| Mu-Protein | Operator-only secure asset intake/conversion tooling implemented for commit `c228769f635e066d592838018b14463720956455`. Both official 8.1 GB checkpoints and the pinned normalization CSV are identified; conversion is hash-locked, uses role-specific `weights_only` allowlists, and emits safetensors without registering an unusable TaskType. | Build the converter, sanitize both checkpoints, construct the pinned legacy inference closure, and reproduce an official score before exposing any TaskType. |
| Pallatom | Dedicated restricted CUDA 12.6/JAX 0.4.34 family implemented for commit `b27d70054dec6ce2f5ceadf8977de3d3baf00663`. The exact checkpoint is external/read-only and the noncommercial access policy is attached. | Complete the final SIF rebuild, run live acceptance under the noncommercial entitlement, then enable only for entitled users. |
| GeoDock | Dedicated CUDA 11.8/Torch 2.0.1/PyG family implemented for commit `df8d1f4c24ae2946655f27e7411ba2ffabf3d350`; its bundled checkpoint and ESM2 assets are provisioned read-only with exact hashes. | Build/test the SIF, but require explicit operator/legal acceptance of checkpoint/model-weight terms before live inference or enablement. |
...
- Eight candidate projects remain on the wait list pending final image/live validation. Protenix's official asset host still cannot complete a TLS handshake through the configured proxy. RFdiffusion2, Foundry, and GeoDock require explicit checkpoint/model-weight use clearance; Mu-Protein requires safe conversion and scientific score reproduction.
