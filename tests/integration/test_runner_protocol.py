...
RUNNER_SCRIPT = Path(__file__).resolve().parents[2] / "docker" / "runners" / "pssm_gremlin" / "run.sh"
GREMLIN_SCRIPT = RUNNER_SCRIPT.parent / "scripts" / "GREMLIN_TFv1.py"
OPENDDE_RUNNER_SCRIPT = Path(__file__).resolve().parents[2] / "docker" / "runners" / "opendde" / "run.sh"
MPNN_RUNNER_SCRIPT = Path(__file__).resolve().parents[2] / "docker" / "runners" / "mpnn" / "run.sh"
DYNAMICMPNN_RUNNER_SCRIPT = Path(__file__).resolve().parents[2] / "docker" / "runners" / "dynamicmpnn" / "run.sh"
ALPHAFOLD_RUNNER_SCRIPT = Path(__file__).resolve().parents[2] / "docker" / "runners" / "alphafold" / "run.sh"
COLABFOLD_RUNNER_SCRIPT = Path(__file__).resolve().parents[2] / "docker" / "runners" / "colabfold_af2" / "run.sh"
ESMDYNAMIC_RUNNER_SCRIPT = Path(__file__).resolve().parents[2] / "docker" / "runners" / "esmdynamic" / "run.sh"
BIOEMU_RUNNER_SCRIPT = Path(__file__).resolve().parents[2] / "docker" / "runners" / "bioemu" / "run.sh"
ESM_RUNNER_SCRIPT = Path(__file__).resolve().parents[2] / "docker" / "runners" / "esm" / "run.sh"
...
def _run_with_manifest(script, input_file, output_dir, env, params=None, extra_args=()):
    ...
    family = Path(script).parent.name
    role = {
        "alphafold": "sequence",
        "bioemu": "sequence",
        "colabfold_af2": "sequence",
        "dynamicmpnn": "structure",
        "esm": "sequence",
        "esmdynamic": "sequence",
        "mpnn": "structure",
        "opendde": "specification",
    }[family]
