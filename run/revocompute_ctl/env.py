...
class EnvState:
    ...
    def harden_env_file(self) -> None:
        """Make the env file owner-only before any secret is written to it.

        New files are already created 0600, but an *existing* one may not be:
        older setup copied the tracked `.env.example` (0644) without tightening
        it, and this file is where generated secrets — the Redis password and
        the session-signing key — are appended.  A permissive mode on an
        upgrade would put the signing key in a broadly readable file, so
        tighten before appending rather than assuming the mode.
        """
        ...
        if current != 0o600:
            os.chmod(path, 0o600)
            print(f"Tightened {self.env_file} to mode 0600 before writing generated secrets.")
    ...
    def ensure_redis_password(self, write: bool = True) -> str:
        ...
        if existing:
            return existing
        persisted = self._persisted_value("REDIS_PASSWORD")
        if persisted:
    ...
    def ensure_auth_secret_key(self, write: bool = True) -> str:
        """Reuse a persisted session-signing key; generate and append one if
        absent.  Without it the web container would mint a fresh key on every
        start, silently logging out every session and invalidating every
        outstanding verification/reset link.  The value is never printed.
        write=False (--dry-run) only reads — nothing is appended."""
        existing = self.get("AUTH_SECRET_KEY")
        if existing:
            return existing
        persisted = self._persisted_value("AUTH_SECRET_KEY")
        if persisted:
            self.values["AUTH_SECRET_KEY"] = persisted
            self.runtime["AUTH_SECRET_KEY"] = persisted
            return persisted
        if not write:
            return ""
        ...
        self.harden_env_file()
        with open(self.env_file, "a", encoding="utf-8") as handle:
            handle.write(f"\n# Generated by restart.sh — auth session-signing key.\nAUTH_SECRET_KEY={key}\n")
        self.values["AUTH_SECRET_KEY"] = key
        self.runtime["AUTH_SECRET_KEY"] = key
        print(f"Generated AUTH_SECRET_KEY and stored it in {self.env_file}.")
        return key

    def _persisted_value(self, key: str) -> str:
        try:
