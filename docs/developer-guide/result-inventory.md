...
| ColabFold AF2 | `1ea9e9178737f7fbb7d3498ae05d31c6` / 171 s | ranked PDB, pLDDT series, PAE matrix/images, pTM/max-PAE scalars, and A3M; for complexes also per-chain-pair ipSAE/pDockQ/pDockQ2 | ranked filename order; pLDDT 0–100 and pTM higher are better; PAE Å lower is better; one-based sequence positions; null means missing; interface scores are dimensionless and exist only when there are at least two chains | candidates + metric series + matrix + scalar summary + alignment + optional interface confidence |
